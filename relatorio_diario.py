...
TOKEN = os.environ["KOMMO_TOKEN"]                 # token da Komma
...
def get_eventos():
      ev, page = [], 1
    while True:
              params = {"filter[created_at][from]": ts0, "filter[created_at][to]": ts1,
                                          "page": page, "limit": 100}
...
for e in eventos:
    ...
    linhas.append({"usuario": nome, "tipo": tipo, "destino_etapa": destino,
                                      "data": dt.date(), "hora": dt.hour})
...
if resumo_rows:
    ...
    if not ws.get_all_values():
              ws.append_row(["data", "pessoa", "eventos_totais",
                                                    "agendamentos", "compareceu", "faltou"])
